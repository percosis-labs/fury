...
import (
	...
	"github.com/percosis-labs/fury/tests/e2e/contracts/greeter"
	"github.com/percosis-labs/fury/tests/util"
)
...
func (suite *IntegrationTestSuite) TestEip712ConvertToCoinAndDepositToLend() {
	...
	principal := sdk.NewCoin("usdf", sdk.NewInt(10e6))
	sdkDenom := suite.DeployedErc20.CosmosDenom

	// create new funded account
	depositor := suite.Fury.NewFundedAccount("eip712-lend-depositor", sdk.NewCoins(ufury(1e5)))
	...
	msgs := []sdk.Msg{
		// convert to coin
		&convertMsg,
		// deposit into cdp (Mint), take out USDF
		&depositMsg,
	}
	...
	tx := suite.NewEip712TxBuilder(
		...
		sdk.NewCoins(ufury(1e4)),
		msgs,
		"doing the USDT Earn workflow! erc20 -> sdk.Coin -> USDF jinx deposit",
	).GetTx()
	...
	cdpRes, err := suite.Fury.Cdp.Cdp(context.Background(), &cdptypes.QueryCdpRequest{
	...
	withdrawAndConvertBack := util.FuryMsgRequest{
		...
		GasLimit:  1e6,
		FeeAmount: sdk.NewCoins(ufury(1000)),
