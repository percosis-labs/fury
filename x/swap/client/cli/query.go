...
func queryDepositsCmd(queryRoute string) *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: strings.TrimSpace(`get liquidity provider deposits:
 		Example:
 		$ fucli q swap deposits --pool bnb:usdf
 		$ fucli q swap deposits --owner fury1l0xsq2z7gqd7yly0g40y5836g0appumark77ny
 		$ fucli q swap deposits --pool bnb:usdf --owner fury1l0xsq2z7gqd7yly0g40y5836g0appumark77ny
 		$ fucli q swap deposits --page=2 --limit=100
 		`,
		),
		Args: cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
