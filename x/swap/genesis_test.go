...
import (
	...
	"github.com/percosis-labs/fury/x/swap/testutil"
...
func (suite *genesisTestSuite) Test_InitAndExportGenesis() {
	...
	state := types.NewGenesisState(
		types.Params{
			AllowedPools: types.AllowedPools{types.NewAllowedPool("ufury", "usdf")},
			SwapFee:      sdk.MustNewDecFromStr("0.00255"),
		},
		types.PoolRecords{
			types.NewPoolRecord(sdk.NewCoins(sdk.NewCoin("jinx", sdkmath.NewInt(1e6)), sdk.NewCoin("usdf", sdkmath.NewInt(2e6))), sdkmath.NewInt(1e6)),
			types.NewPoolRecord(sdk.NewCoins(sdk.NewCoin("ufury", sdkmath.NewInt(1e6)), sdk.NewCoin("usdf", sdkmath.NewInt(5e6))), sdkmath.NewInt(3e6)),
		},
		types.ShareRecords{
			types.NewShareRecord(depositor_2, types.PoolID("jinx", "usdf"), sdkmath.NewInt(1e6)),
			types.NewShareRecord(depositor_1, types.PoolID("ufury", "usdf"), sdkmath.NewInt(3e6)),
		},
	)
	...
	poolRecord1, _ := suite.Keeper.GetPool(suite.Ctx, types.PoolID("jinx", "usdf"))
	suite.Equal(state.PoolRecords[0], poolRecord1)
	poolRecord2, _ := suite.Keeper.GetPool(suite.Ctx, types.PoolID("ufury", "usdf"))
	suite.Equal(state.PoolRecords[1], poolRecord2)

	shareRecord1, _ := suite.Keeper.GetDepositorShares(suite.Ctx, depositor_2, types.PoolID("jinx", "usdf"))
	suite.Equal(state.ShareRecords[0], shareRecord1)
	shareRecord2, _ := suite.Keeper.GetDepositorShares(suite.Ctx, depositor_1, types.PoolID("ufury", "usdf"))
