    ...
    "params": {
		...
		"pool_records": [
			...
			},
		  {
			  "pool_id": "jinx:usdf",
			  "reserves_a": { "denom": "ufury", "amount": "1000000" },
			  "reserves_b": { "denom": "usdf", "amount": "2000000" },
	...
	state := types.NewGenesisState(
		types.NewParams(
			types.NewAllowedPools(
				types.NewAllowedPool("ufury", "usdf"),
				types.NewAllowedPool("jinx", "busd"),
			),
			sdk.MustNewDecFromStr("0.003"),
		),
		types.PoolRecords{
			types.NewPoolRecord(sdk.NewCoins(ufury(1e6), usdf(5e6)), i(3e6)),
			types.NewPoolRecord(sdk.NewCoins(jinx(1e6), usdf(2e6)), i(15e5)),
		},
		types.ShareRecords{
			types.NewShareRecord(depositor_1, types.PoolID("ufury", "usdf"), i(1e5)),
			types.NewShareRecord(depositor_2, types.PoolID("jinx", "usdf"), i(2e5)),
		},
	)
...
func TestGenesis_ValidatePoolRecords(t *testing.T) {
	invalidPoolRecord := types.NewPoolRecord(sdk.NewCoins(ufury(1e6), usdf(5e6)), i(-1))
...
func TestGenesis_Validate_PoolShareIntegration(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			shareRecords: types.ShareRecords{},
			expectedErr:  "total depositor shares 0 not equal to pool 'ufury:usdf' total shares 3000000",
		},
		{
			name:        "zero pool records, one share record",
			poolRecords: types.PoolRecords{},
			shareRecords: types.ShareRecords{
				types.NewShareRecord(depositor_1, types.PoolID("ufury", "usdf"), i(5e6)),
			},
			expectedErr: "total depositor shares 5000000 not equal to pool 'ufury:usdf' total shares 0",
		},
		{
			...
			shareRecords: types.ShareRecords{
				types.NewShareRecord(depositor_1, "ufury:usdf", i(15e5)),
			},
			expectedErr: "total depositor shares 1500000 not equal to pool 'ufury:usdf' total shares 3000000",
		},
		{
			name: "more than one pool records, more than one share record",
			poolRecords: types.PoolRecords{
				types.NewPoolRecord(sdk.NewCoins(ufury(1e6), usdf(5e6)), i(3e6)),
				types.NewPoolRecord(sdk.NewCoins(jinx(1e6), usdf(2e6)), i(2e6)),
			},
			shareRecords: types.ShareRecords{
				types.NewShareRecord(depositor_1, types.PoolID("ufury", "usdf"), i(15e5)),
				types.NewShareRecord(depositor_2, types.PoolID("ufury", "usdf"), i(15e5)),
				types.NewShareRecord(depositor_1, types.PoolID("jinx", "usdf"), i(1e6)),
			},
			expectedErr: "total depositor shares 1000000 not equal to pool 'jinx:usdf' total shares 2000000",
		},
		{
			...
			shareRecords: types.ShareRecords{
				types.NewShareRecord(depositor_1, types.PoolID("ufury", "usdf"), i(15e5)),
				types.NewShareRecord(depositor_2, types.PoolID("ufury", "usdf"), i(15e5)),
				types.NewShareRecord(depositor_1, types.PoolID("jinx", "usdf"), i(2e6)),
