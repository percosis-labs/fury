...
func NewQuerier(bk types.BankKeeper, legacyQuerierCdc *codec.LegacyAmino) sdk.Querier {
	return func(ctx sdk.Context, path []string, req abci.RequestQuery) (res []byte, err error) {
		...
		case types.QueryCirculatingSupplyJINX:
			return getCirculatingSupplyJINX(ctx, req, legacyQuerierCdc)
		case types.QueryCirculatingSupplyUSDF:
			return getCirculatingSupplyUSDF(ctx, req, bk, legacyQuerierCdc)
		case types.QueryCirculatingSupplyMER:
			return getCirculatingSupplyMER(ctx, req, legacyQuerierCdc)
		case types.QueryTotalSupplyJINX:
			return getTotalSupplyJINX(ctx, req, bk, legacyQuerierCdc)
		case types.QueryTotalSupplyUSDF:
			return getCirculatingSupplyUSDF(ctx, req, bk, legacyQuerierCdc) // Intentional - USDF total supply is the circulating supply
		default:
...
func getCirculatingSupplyJINX(ctx sdk.Context, req abci.RequestQuery, legacyQuerierCdc *codec.LegacyAmino) ([]byte, error) {
...
func getCirculatingSupplyUSDF(ctx sdk.Context, req abci.RequestQuery, bk types.BankKeeper, legacyQuerierCdc *codec.LegacyAmino) ([]byte, error) {
	totalSupply := bk.GetSupply(ctx, "usdf").Amount
	supplyInt := sdk.NewDecFromInt(totalSupply).Mul(sdk.MustNewDecFromStr("0.000001")).TruncateInt64()
	bz, err := legacyQuerierCdc.MarshalJSON(supplyInt)
	if err != nil {
		return nil, errorsmod.Wrap(sdkerrors.ErrJSONUnmarshal, err.Error())
...
func getCirculatingSupplyMER(ctx sdk.Context, req abci.RequestQuery, legacyQuerierCdc *codec.LegacyAmino) ([]byte, error) {
	// Start values
	year := 2021
	month := 8
	...
	teamMer := int64(4_687_500)
	treasuryMer := int64(5_859_375)
	monthlyStakersMer := int64(520_833)
	monthlyLPIncentivesMer := int64(2_343_750)

	// []{Ecosystem, Team, Treasury, Fury Stakers, LP Incentives}
	scheduleAmounts := [][]int64{
		{12_500_000, 0, 15_625_000, monthlyStakersMer, monthlyLPIncentivesMer},  // *** Year ONE ***
		{0, 0, 0, monthlyStakersMer, monthlyLPIncentivesMer},                    // 1
		{0, 0, 0, monthlyStakersMer, monthlyLPIncentivesMer},                    // 2
		{0, 0, treasuryMer, monthlyStakersMer, monthlyLPIncentivesMer},          // 3
		{0, 0, 0, monthlyStakersMer, monthlyLPIncentivesMer},                    // 4
		{0, 0, 0, monthlyStakersMer, monthlyLPIncentivesMer},                    // 5
		{0, 0, treasuryMer, monthlyStakersMer, monthlyLPIncentivesMer},          // 6
		{0, 0, 0, monthlyStakersMer, monthlyLPIncentivesMer},                    // 7
		{0, 0, 0, monthlyStakersMer, monthlyLPIncentivesMer},                    // 8
		{0, 0, treasuryMer, monthlyStakersMer, monthlyLPIncentivesMer},          // 9
		{0, 0, 0, monthlyStakersMer, monthlyLPIncentivesMer},                    // 10
		{0, 0, 0, monthlyStakersMer, monthlyLPIncentivesMer},                    // 11
		{0, 18_750_000, treasuryMer, monthlyStakersMer, monthlyLPIncentivesMer}, // *** Year TWO ***
		{0, 0, 0, monthlyStakersMer, monthlyLPIncentivesMer},                    // 13
		{0, 0, 0, monthlyStakersMer, monthlyLPIncentivesMer},                    // 14
		{0, teamMer, treasuryMer, monthlyStakersMer, monthlyLPIncentivesMer},    // 15
		{0, 0, 0, monthlyStakersMer, monthlyLPIncentivesMer},                    // 16
		{0, 0, 0, monthlyStakersMer, monthlyLPIncentivesMer},                    // 17
		{0, teamMer, treasuryMer, monthlyStakersMer, monthlyLPIncentivesMer},    // 18
		{0, 0, 0, monthlyStakersMer, monthlyLPIncentivesMer},                    // 19
		{0, 0, 0, monthlyStakersMer, monthlyLPIncentivesMer},                    // 20
		{0, teamMer, treasuryMer, monthlyStakersMer, monthlyLPIncentivesMer},    // 21
		{0, 0, 0, monthlyStakersMer, monthlyLPIncentivesMer},                    // 22
		{0, 0, 0, monthlyStakersMer, monthlyLPIncentivesMer},                    // 23
		{0, teamMer, treasuryMer, monthlyStakersMer, monthlyLPIncentivesMer},    // *** Year THREE ***
	}

	// Months 25-47 are the same
	for i := 0; i < 23; i++ {
		scheduleAmounts = append(scheduleAmounts, []int64{0, 0, 0, monthlyStakersMer, monthlyLPIncentivesMer})
	}
...
func getTotalSupplyJINX(ctx sdk.Context, req abci.RequestQuery, bk types.BankKeeper, legacyQuerierCdc *codec.LegacyAmino) ([]byte, error) {
