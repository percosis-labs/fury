...
func (suite *HandlerTestSuite) authBuilder() *app.AuthBankGenesisBuilder {
	return app.NewAuthBankGenesisBuilder().
		WithSimpleModuleAccount(furydisttypes.ModuleName, cs(c(types.USDFMintingRewardDenom, 1e18), c("jinx", 1e18), c("swap", 1e18)))
}
...
func (suite *HandlerTestSuite) TestPayoutSwapClaimMultiDenom() {
	...
	expectedRewardsJinx := c("jinx", int64(0.2*float64(7*1e6)))
