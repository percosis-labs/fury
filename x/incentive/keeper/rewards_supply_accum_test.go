...
func (suite *AccumulateSupplyRewardsTests) storedTimeEquals(denom string, expected time.Time) {
	storedTime, found := suite.keeper.GetPreviousJinxSupplyRewardAccrualTime(suite.ctx, denom)
	suite.True(found)
...
func (suite *AccumulateSupplyRewardsTests) storedIndexesEqual(denom string, expected types.RewardIndexes) {
	storedIndexes, found := suite.keeper.GetJinxSupplyRewardIndexes(suite.ctx, denom)
...
func (suite *AccumulateSupplyRewardsTests) TestStateUpdatedWhenBlockTimeHasIncreased() {
	...
	jinxKeeper := newFakeJinxKeeper().addTotalSupply(c(denom, 1e6), d("1"))
	...
	suite.keeper.SetPreviousJinxSupplyRewardAccrualTime(suite.ctx, denom, previousAccrualTime)
	...
	suite.keeper.AccumulateJinxSupplyRewards(suite.ctx, period)
...
func (suite *AccumulateSupplyRewardsTests) TestNoAccumulationWhenSourceSharesAreZero() {
	...
	jinxKeeper := newFakeJinxKeeper() // zero total supplys
	suite.keeper = suite.NewKeeper(&fakeParamSubspace{}, nil, nil, jinxKeeper, nil, nil, nil, nil, nil, nil)
