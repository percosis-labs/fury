...
func TestKvCLIKeysAddRecover(t *testing.T) {
	...
	require.Equal(t, "fury1rsjxn2e4dfl3a2qzuzzjvvgjmmate383g9q4cz", f.KeyAddress("test-recover").String())
...
func TestFuryCLIKeysAddRecoverHDPath(t *testing.T) {
	t.Parallel()
	f := InitFixtures(t)
	...
	require.Equal(t, "fury1rsjxn2e4dfl3a2qzuzzjvvgjmmate383g9q4cz", f.KeyAddress("test-recoverHD1").String())
	...
	require.Equal(t, "fury1qpj6nstqn0n5gzcsaezspuhulje6msjq5t8cq5", f.KeyAddress("test-recoverH2").String())
	...
	require.Equal(t, "fury1vayfpstgapt7dmv7074kc3ll8xpf0rlzvh4k08", f.KeyAddress("test-recoverH3").String())
	...
	require.Equal(t, "fury1xvsfnksmhr887skcfrm4pe3va54tkmrtw7wyer", f.KeyAddress("test-recoverH4").String())
	...
	require.Equal(t, "fury1v9plmhvyhgxk3th9ydacm7j4z357s3nhhmy0tv", f.KeyAddress("test-recoverH5").String())
	...
	require.True(t, exitSuccess)
	require.Equal(t, "fury1rsjxn2e4dfl3a2qzuzzjvvgjmmate383g9q4cz", f.KeyAddress("test-recoverH6").String())
	...
	require.True(t, exitSuccess)
	require.Equal(t, "fury1xvsfnksmhr887skcfrm4pe3va54tkmrtw7wyer", f.KeyAddress("test-recoverH7").String())
...
func TestKvCLIMinimumFees(t *testing.T) {
	...
	// start fud server with minimum fees
...
func TestKvCLIQueryRewards(t *testing.T) {
	...
	genFile := filepath.Join(f.FudHome, "config", "genesis.json")
...
func TestKvCLIConfig(t *testing.T) {
	...
	config, err := ioutil.ReadFile(path.Join(f.FucliHome, "config", "config.toml"))
	require.NoError(t, err)
...
func TestFudCollectGentxs(t *testing.T) {
	t.Parallel()
...
func TestFudAddGenesisAccount(t *testing.T) {
	t.Parallel()
	f := NewFixtures(t)
