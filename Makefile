...
HTTPS_GIT := https://github.com/Percosis-Labs/fury.git
...
build: go.sum
ifeq ($(OS), Windows_NT)
	$(GO_BIN) build -mod=readonly $(BUILD_FLAGS) -o out/$(shell $(GO_BIN) env GOOS)/fury.exe ./cmd/fury
else
	$(GO_BIN) build -mod=readonly $(BUILD_FLAGS) -o out/$(shell $(GO_BIN) env GOOS)/fury ./cmd/fury
endif
...
install: go.sum
	$(GO_BIN) install -mod=readonly $(BUILD_FLAGS) ./cmd/fury
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -name '*.pb.go' | xargs goimports -w -local github.com/percosis-labs/fury
.PHONY: format
...
contract/artifacts/Bridge.abi: contract/contracts/Bridge.sol
	cd contract && $(SOLC) --abi contracts/Bridge.sol --base-path . --include-path node_modules/ -o artifacts --overwrite

contract/artifacts/Bridge.bin: contract/contracts/Bridge.sol
	cd contract && $(SOLC) --optimize --bin contracts/Bridge.sol --base-path . --include-path node_modules/ -o artifacts --overwrite
...
contract/artifacts/node_modules_@openzeppelin_contracts_token_ERC20_ERC20_sol_ERC20.abi: contract/node_modules/@openzeppelin/contracts/token/ERC20/ERC20.sol
	cd contract && $(SOLC) --abi node_modules/@openzeppelin/contracts/token/ERC20/ERC20.sol --base-path . --include-path node_modules/ -o artifacts --overwrite

relayer/bridge.go: contract/artifacts/Bridge.bin contract/artifacts/Bridge.abi
	$(ABIGEN) --bin $< --abi $(word 2,$^) --pkg relayer --type Bridge --out $@ --overwrite

relayer/erc20.go: contract/artifacts/ERC20.abi
...
test-rocksdb:
	@go test -tags=rocksdb ./cmd/fury/opendb

# Run cli integration tests
# `-p 4` to use 4 cores, `-tags cli_test` to tell $(GO_BIN) not to ignore the cli package
# These tests use the `fud` or `fucli` binaries in the build dir, or in `$BUILDDIR` if that env var is set.
test-cli: build
